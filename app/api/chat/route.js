...
function extractLeadData(messages) {
  ...
  const nameMatch = conversation.match(
    /(?:my name is|i am|i'm|name:|called)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/i
  );
  const companyMatch = conversation.match(
    /(?:company|business|work at|from|represent)\s+([A-Z][a-zA-Z\s]+?)(?:\.|,|\s+and|\s+we)/i
  );
...
6. Based on their answers, explain which of our 5 services would help them:
   ...
   - ⚙️ Make.com Automation Engine (workflow automation)
...
- Add this to every first message: "📋 By chatting with us, you agree that your information may be used to contact you about our services, in accordance with POPIA."`,
 ...
 ({
    ...
    // Send to Make.com if lead is qualified
