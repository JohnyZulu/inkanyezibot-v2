...
export async function runAgentWithTools(userInstruction, tools) {
  ...
  // Cap the number of tool-call ROUNDS (one round can contain several tool
  // calls). Prevents a runaway loop. (#11 renamed for clarity.)
  const MAX_TOOL_ROUNDS = 5;
  let aiMsg = await modelWithTools.invoke(messages);
  messages.push(aiMsg);

  let round = 0;
  while (aiMsg.tool_calls && aiMsg.tool_calls.length > 0 && round < MAX_TOOL_ROUNDS) {
    round += 1;
    for (const toolCall of aiMsg.tool_calls) {
      ...
      try {
        ...
        // #12: log the real error server-side; give the model only a stable,
        // non-sensitive message (raw errors may carry internal detail once
        // Sheets/network integrations exist). #6: safe on non-Error throws.
        console.error(`Tool "${toolCall.name}" threw:`, toolErr);
        messages.push({
          role: "tool",
          tool_call_id: toolCall.id,
          content: `The tool "${toolCall.name}" encountered an internal error and did not complete. Do not retry blindly; report this if it persists.`,
        });
      }
    }
  ...
  // #11: if we exited because the round cap was hit while the model still
  // wanted to call tools, say so explicitly rather than returning empty content.
  if (aiMsg.tool_calls && aiMsg.tool_calls.length > 0) {
    return `I reached the maximum number of tool steps (${MAX_TOOL_ROUNDS}) before finishing this task. Some requested actions may not have completed. Please narrow the task or try again.`;
  }
