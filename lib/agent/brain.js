...
export async function runAgentWithTools(userInstruction, tools) {
  ...
  while (aiMsg.tool_calls && aiMsg.tool_calls.length > 0 && step < MAX_STEPS) {
    ...
    for (const toolCall of aiMsg.tool_calls) {
      ...
      // with the matching tool_call_id. We wrap execution so that if a tool
      // throws (bad input, network error in Stage 2, etc.) the model gets a
      // readable error to recover from, instead of the whole request crashing.
      try {
        ...
        messages.push({
          role: "tool",
          tool_call_id: toolCall.id,
          content: `Error running tool "${toolCall.name}": ${toolErr.message}. Check your inputs and try again if appropriate.`,
        });
      }
    }
