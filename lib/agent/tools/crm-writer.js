...
export const crmWriterTool = tool(
  async ({ reference, status, score, notes }) => {
    try {
      ...
      // Log the real error server-side; return only a stable, non-sensitive
      // message to the model (closes the tool-local error-leak bypass).
      console.error(`update_crm unexpected error for ${reference}:`, err);
      return JSON.stringify({
        success: false,
        error: "An unexpected error occurred and no change was confirmed. Do not retry blindly.",
      });
    }
  },
  {
    ...
    schema: z.object({
      ...
      score: z
        .enum(ALLOWED_SCORES)
        .optional()
        .describe("Lead score: HOT, WARM, or COLD."),
      notes: z
        .string()
        .max(2000)
        .optional()
